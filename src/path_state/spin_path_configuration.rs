...
/// SpinPathConfiguration struct to store particle positions using ndarray.
///
/// # Example
/// ```
/// use pimc::path_state::spin_path_configuration::SpinPathConfiguration;
///
/// // Create a new SpinPathConfiguration instance
/// let mut world = SpinPathConfiguration::<2,3,3>::new(); // 2 particles, 3 time slices, 3D space
///
...
pub struct SpinPathConfiguration<const N: usize, const M: usize, const D: usize> {
...
impl<const N: usize, const M: usize, const D: usize> Default for SpinPathConfiguration<N, M, D> {
    fn default() -> Self {
        Self::new()
    }
}

impl<const N: usize, const M: usize, const D: usize> SpinPathConfiguration<N, M, D> {
    /// Creates a new `SpinPathConfiguration` instance with all positions initialized to zero.
    pub fn new() -> Self {
        Self {
            positions: Array::zeros((N, M, D)),
            spins: Array::from_elem((N, M), Spin::Up),
    ...
    /// use pimc::path_state::spin_path_configuration::SpinPathConfiguration;
    /// use ndarray::array;
    /// use pimc::path_state::spin::Spin;
    ///
    /// let mut world = SpinPathConfiguration::<2, 3, 3>::new();
    ///
    /// // Initialize positions and spins with a simple function
    /// world.initialize_positions_spins(|particle| vec![particle as f64; 3], |particle| if
    /// particle % 2 == 0 {Spin::Up} else {Spin::Dn} );
    ///
    /// // Verify initialization
    /// assert_eq!(world.position(0, 0), array![0.0, 0.0, 0.0]);
    /// assert_eq!(world.position(1, 1), array![1.0, 1.0, 1.0]);
    /// ```
    pub fn initialize_positions_spins<F, S>(&mut self, position_generator: F, spin_generator: S)
    where
        F: Fn(usize) -> Vec<f64>,
        S: Fn(usize) -> Spin,
    {
        for particle in 0..N {
            ...
            let spin = spin_generator(particle);
            ...
            for time_slice in 0..M {
                ...
                self.set_spin(particle, time_slice, spin);
            }
        }
    }

    /// Saves the `SpinPathConfiguration` instance to a file in JSON format.
    ///
    /// # Arguments
    ...
    /// Loads a `SpinPathConfiguration` instance from a JSON file.
    ///
    /// # Arguments
    ...
    /// use ndarray::array;
    ///
    /// let mut world = SpinPathConfiguration::<2,5,3>::new(); // 2 particles, 5 time slices, 3D space
    /// world.set_position(0, 0, &[1.0, 2.0, 3.0]);
    ...
    /// Gets the spin state of a specific particle at a specific time slice.
    ///
    /// # Arguments
    ...
    ///
    /// # Returns
    /// The `Spin` of the specified particle at the given time slice.
    ///
    /// # Panics
    /// Panics if the `particle` or `time_slice` indices are out of bounds.
    pub fn spin(&self, particle: usize, time_slice: usize) -> Spin {
        debug_assert!(
            particle < N,
            "Particle index out of bounds: particle={}, max allowed={}",
            particle,
        ...
        self.spins[[particle, time_slice]]
    }

    /// Sets the spin state of a specific particle at a specific time slice.
    ///
    /// # Arguments
    /// * `particle` - The index of the particle.
    /// * `time_slice` - The index of the time slice.
    /// * `new_spin` - The `Spin` to assign to the particle.
    pub fn set_spin(&mut self, particle: usize, time_slice: usize, new_spin: Spin) {
        debug_assert!(
            particle < N,
            "Particle index out of bounds: particle={}, max allowed={}",
            particle,
        ...
        self.spins[[particle, time_slice]] = new_spin;
    }

    /// Flips the spin of a specific particle at a specific time slice.
    ///
    /// # Arguments
    /// * `particle` - The index of the particle.
    /// * `time_slice` - The index of the time slice.
    pub fn flip_spin(&mut self, particle: usize, time_slice: usize) {
        debug_assert!(
            particle < N,
            "Particle index out of bounds: particle={}, max allowed={}",
            particle,
        ...
        self.spins[[particle, time_slice]].flip();
    }
...
mod tests {
    ...
    fn test_new_worm() {
        ...
        let world = SpinPathConfiguration::<2, 3, 3>::new(); // 2 particles, 3 time slices, 3D
    ...
    fn test_invalid_arguments_position_mut() {
        let mut world = SpinPathConfiguration::<2, 3, 2>::new();
    ...
    fn test_particle_spin() {
        ...
        let mut world = SpinPathConfiguration::<N, M, D>::new();

        // Default state should be Up
        for particle in 0..N {
            for time_slice in 0..M {
                assert_eq!(world.spin(particle, time_slice), Spin::Up);
            }
        }

        for time_slice in (0..M).step_by(2) {
            // Cange the particle state to Dn for particle 1 at even slices
            world.set_spin(1, time_slice, Spin::Dn)
        }
    ...
    fn test_set_permutations() {
        ...
        const NS: usize = 16;
        const ND: usize = 3;
        let mut world = SpinPathConfiguration::<NP, NS, ND>::new();
    ...
    fn test_save_and_load_json_temp() -> io::Result<()> {
        ...
        let loaded_world =
            SpinPathConfiguration::<2, 3, 3>::load_from_file(temp_file.path().to_str().unwrap())?;
